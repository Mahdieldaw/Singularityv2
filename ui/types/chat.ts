...
export const convertTurnMessageToChatTurn = (turn: TurnMessage): ChatTurn => {
  if (turn.type === "user") {
    ...
    Object.entries(aiTurn.batchResponses || {}).forEach(
      ([providerId, response]) => {
        responses.push({
    ...
    Object.entries(aiTurn.synthesisResponses || {}).forEach(
      ([providerId, responseArray]) => {
    ...
    Object.entries(aiTurn.mappingResponses || {}).forEach(
      ([providerId, responseArray]) => {
        responseArray.forEach((response, index) => {
          responses.push({
...
export const convertTurnMessagesToChatTurns = (
  turns: TurnMessage[],
): ChatTurn[] => {
...
export const getPrimaryResponseContent = (turn: ChatTurn): string => {
  ...
  const batchResponse = turn.responses.find(
    (r) =>
      !r.providerId.includes("-synthesis") &&
      !r.providerId.includes("-mapping"),
  );
  if (batchResponse) {
    return batchResponse.content;
  }

  const synthesisResponse = turn.responses.find((r) =>
    r.providerId.includes("-synthesis"),
  );
...
export const getResponseByProvider = (
  turn: ChatTurn,
  providerId: string,
  ...
  return Array.from(
    new Set(
      turn.responses.map((r) =>
        r.providerId.replace("-synthesis", "").replace("-mapping", ""),
      ),
    ),
  );
};
